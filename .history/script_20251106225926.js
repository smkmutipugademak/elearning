...
function renderMapel() {
    ...
    filtered.forEach(m => {
      ...
      <h4>${m.title}</h4>
      <p>${m.kode}</p>
      <button class="btn-go" onclick="openMapel('${m.title}')">Lihat Detail</button>
    `;
...
function openMapel(mapel) {
    alert(`Membuka ${mapel}\\nSilakan pilih Materi, Praktikum, Latihan Soal, atau TryOut`);
    // nanti diarahkan ke halaman materi.html?mapel=...
}
