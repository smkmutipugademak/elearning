...
function showBab(mapel) {
    ...
    babListData.forEach((bab, index) => {
        ...
        div.addEventListener("click", () => {
            // buat nama file otomatis, misalnya:
            // mapel "Coding" + Bab 1 => latihan_coding_bab1.html
            const mapelSlug = mapel.toLowerCase().replace(/\s+/g, "_");
            const babSlug = bab.toLowerCase().replace("latihan bab ", "bab");
            const targetFile = `latihan_${mapelSlug}_${babSlug}.html`;

            // redirect ke file latihan sesuai mapel & bab
